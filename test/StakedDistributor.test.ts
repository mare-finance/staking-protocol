...
async function deployTokensFixture() {
    ...
    // Mare
    const Mare = await ethers.getContractFactory('MockERC20Token');
    const mare = await Mare.connect(admin).deploy(ethers.utils.parseUnits('10000', 18), 18);

    // Send some Mare to the staker0 and staker1
    await (await mare.connect(admin).transfer(staker0.address, ethers.utils.parseUnits('10', 18))).wait(1);
    await (await mare.connect(admin).transfer(staker1.address, ethers.utils.parseUnits('10', 18))).wait(1);
    ...
    const stakedDistributor = await StakedDistributor.connect(admin).deploy(mare.address, 'SS', 'ss');
    ...
    await (await mare.connect(staker0).approve(stakedDistributor.address, totalStakeAmountsByStaker[0])).wait(1);
    await (await stakedDistributor.connect(staker0).mint(stakeAmounts[0][0])).wait(1);

    // Add Initial Stakers: 2
    await (await mare.connect(staker1).approve(stakedDistributor.address, totalStakeAmountsByStaker[1])).wait(1);
...
describe('Staked Distributor', function () {
    ...
    it('Should withdraw burned tokens', async function () {
        const { stakedDistributor, mare, staker0, staker1 } = await loadFixture(deployTokensFixture);
        ...
        expect(await mare.balanceOf(staker0.address)).to.be.equal(staker0Mare.add(toBurn0));
        ...
        expect(await mare.balanceOf(staker1.address)).to.be.equal(staker1Mare.add(toBurn1));
    });
